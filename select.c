...
// algoritmo ray casting: ponto dentro do polígono?
...
// testa se (x,y) está sobre/na figura
...
void selectShape(Selector *sel, Shape **shapes, int shapeCount, float x, float y) {
    ...
    // percorre de trás pra frente (último desenhado tem prioridade)
    for (int i = shapeCount - 1; i >= 0; i--) {
        Shape *s = shapes[i];
        if (isInsideShape(x, y, s)) {
            sel->selected = s;
            sel->index = i;
            printf("Figura selecionada (tipo=%d, index=%d)\n", s->type, i);
            return;
        }
    }
}
...
void deleteSelected(Selector *sel, Shape **shapes, int *shapeCount) {
    if (!sel->selected || sel->index < 0) return;
